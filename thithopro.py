...
# --- CẤU HÌNH GIAO DIỆN ---
st.set_page_config(page_title="ThiTho Pro - Multi-AI Edition", layout="wide", initial_sidebar_state="expanded")
...
# --- HÀM XỬ LÝ NHIỀU AI (MULTI-AI STRATEGY) ---
def get_ai_explanation(q, options, corr_text):
    try:
        genai.configure(api_key=HIDDEN_API_KEY)
        
        # Lấy danh sách các model đang hoạt động trong API Key của bạn
        ...
        prompt = f"""Bạn là một chuyên gia phản biện về Mạng máy tính và Lập trình.
        Câu hỏi: {q}
        Đáp án tài liệu chọn: {corr_label}. {corr_text}

        NHIỆM VỤ:
        1. Thẩm định kiến thức: Nếu đáp án tài liệu chọn SAI, hãy ghi "⚠️ CẢNH BÁO: Đáp án trong tài liệu có thể chưa chính xác!".
        2. Phân tích chi tiết lý do kỹ thuật.
        3. Giải thích tại sao các phương án khác không đúng.
        Trình bày chuyên nghiệp, gạch đầu dòng rõ ràng."""

        # Thử lần lượt các AI cho đến khi có kết quả
        ...
        return "❌ Tất cả các mô hình AI đều đang bận hoặc không phản hồi."
    except Exception as e: 
    ...
    with col_m:
        ...
        if answered:
            ...
            if st.session_state.user_answers[idx] == item['correct']:
                ...
                st.error(f"Sai rồi! Đáp án đúng: {labels[c_idx]}")
            
            if st.button("🔍 Phân tích chuyên sâu (Đa AI)"):
                with st.spinner("Các AI đang hội ý để phản biện..."):
            ...
            if idx in st.session_state.ex_cache:
                ...
                st.markdown(f'<div class="{box_class}"><b>🤖 Hội đồng AI Phân tích:</b><br><br>{explanation}</div>', unsafe_allow_html=True)
